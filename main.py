...
# attendance_logs.video_path / area_name / region_name
        ...
        # daily_settings payroll knobs
        for col, ddl in [
            ("late_penalty_percent_per_hour", "ALTER TABLE daily_settings ADD COLUMN late_penalty_percent_per_hour FLOAT NOT NULL DEFAULT 0"),
            ("overtime_multiplier", "ALTER TABLE daily_settings ADD COLUMN overtime_multiplier FLOAT NOT NULL DEFAULT 1.5"),
            ("bonus_perfect_day", "ALTER TABLE daily_settings ADD COLUMN bonus_perfect_day FLOAT NOT NULL DEFAULT 0"),
            ("early_leave_grace_minutes", "ALTER TABLE daily_settings ADD COLUMN early_leave_grace_minutes INT NOT NULL DEFAULT 5"),
        ...
        # support tickets table (legacy name 'tickets' might exist in older builds)
        conn.execute(text("""CREATE TABLE IF NOT EXISTS support_tickets (
        ...
        conn.execute(text("""CREATE TABLE IF NOT EXISTS attendance_adjustments (
            id INT AUTO_INCREMENT PRIMARY KEY,
            employee_id INT NOT NULL,
            ...
            decision_absence VARCHAR(20) NOT NULL DEFAULT 'PENDING',
            updated_by_user_id INT NULL,
            updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
            UNIQUE KEY uq_emp_day (employee_id, day_date),
            FOREIGN KEY (employee_id) REFERENCES employees(id),
            FOREIGN KEY (updated_by_user_id) REFERENCES users(id)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;"""))


        # payroll adjustments table (manual +/- adjustments per employee per month)
        conn.execute(text("""CREATE TABLE IF NOT EXISTS payroll_adjustments (
            id INT AUTO_INCREMENT PRIMARY KEY,
            employee_id INT NOT NULL,
            ...
            created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
            KEY idx_emp_month (employee_id, month),
            FOREIGN KEY (employee_id) REFERENCES employees(id),
        ...
        try:
            conn.execute(text("""
                CREATE TABLE IF NOT EXISTS attendance_early_leave_segments (
                    id INT AUTO_INCREMENT PRIMARY KEY,
                    employee_id INT NOT NULL,
                    day_date DATE NOT NULL,
                    out_ts DATETIME NOT NULL,
                    in_ts DATETIME NULL,
                    ...
                    updated_by_user_id INT NULL,
                    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                    UNIQUE KEY uq_els_emp_day_out_end (employee_id, day_date, out_ts, end_ts),
                    KEY ix_els_emp_day (employee_id, day_date),
                    CONSTRAINT fk_els_emp FOREIGN KEY (employee_id) REFERENCES employees(id) ON DELETE CASCADE
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
            """))
        except Exception:
            pass
...
def enforce_daily_window(db: Session, when: datetime) -> DailySettings:
    ...
    # Allowed window: [work_start, work_end]
    t_now = when.timetz().replace(tzinfo=None)  # compare naive times
    if t_now < settings.work_start:
        raise HTTPException(status_code=400, detail="التسجيل مغلق الآن (قبل بداية الدوام).")
    if t_now > settings.work_end:
        raise HTTPException(status_code=400, detail="التسجيل مغلق الآن (انتهى الدوام).")
...
def _compute_payroll_for_month(
    ...
    # Official shift is always 9 hours for payroll calculations (per your rule).
    shift_hours = 9.0
...
def _compute_day_row_for_employee(db: Session, emp: Employee, d: date, settings: DailySettings | None, *, write_db: bool = False):
    ...
    if last_out and work_end:
        end_time_dt = datetime.combine(d, work_end)
        last_out_ts = _as_naive(last_out.server_timestamp)
        end_ts = _as_naive(end_time_dt)
        if last_out_ts and end_ts:
    ...
    # Early leave is tracked as segments:
    # - BREAK: OUT -> next IN during scheduled hours
    # - FINAL: last OUT -> scheduled end (with grace)
    early_leave_total_minutes = 0
    ...
    if work_end:
        segs = compute_early_leave_segments(sessions, d, work_end, int(early_leave_grace or 0))
        if segs:
            # In preview mode we must NOT write to DB. If segments were previously committed,
            # we can still read them for decisions.
            existing_recs = (
                db.query(AttendanceEarlyLeaveSegment)
                .filter(AttendanceEarlyLeaveSegment.employee_id == emp.id, AttendanceEarlyLeaveSegment.day_date == d)
                .order_by(AttendanceEarlyLeaveSegment.out_ts.asc(), AttendanceEarlyLeaveSegment.end_ts.asc())
                .all()
            )

            seg_recs = None
            if write_db:
                # upsert + pull decisions
                seg_recs = sync_early_leave_segments(db, emp.id, d, segs)
            elif existing_recs:
                seg_recs = existing_recs

            early_leave_segments = seg_recs or []

            # Raw minutes always come from computed segments.
            early_leave_total_minutes = int(sum(int(s.get("minutes") or 0) for s in segs))

            # Approved minutes only from persisted records (decisions).
            if seg_recs:
                early_leave_approved_minutes = int(
                    sum(int(getattr(r, "minutes", 0) or 0) for r in seg_recs if (getattr(r, "decision", "PENDING") or "PENDING") == "APPROVED")
                )
            else:
                early_leave_approved_minutes = 0

            # For display only: build HH:MM:SS for total minutes
            early_leave_seconds = int(early_leave_total_minutes * 60)
            ...
            # Fallback to legacy single-final-OUT early leave (keeps old installs behaving)
            early_leave_minutes = 0
            if last_out:
                end_time_dt = datetime.combine(d, work_end)
                last_out_ts = _as_naive(last_out.server_timestamp)
                end_ts = _as_naive(end_time_dt)
                if last_out_ts and end_ts:
                    early_before = end_ts - timedelta(minutes=early_leave_grace)
                    if last_out_ts < early_before:
                        early_leave_seconds = int((early_before - last_out_ts).total_seconds())
                        early_leave_minutes = int(math.ceil(early_leave_seconds / 60.0)) if early_leave_seconds > 0 else 0
                        early_leave_total_minutes = int(early_leave_minutes)
                        early_leave_approved_minutes = int(early_leave_minutes)
                        h = early_leave_seconds // 3600
                        rem = early_leave_seconds % 3600
                        mm = rem // 60
                        ss = rem % 60
    ...
    if write_db:
        if not (
            ...
            if raw_late_minutes > 0 or raw_early_leave_minutes > 0 or raw_status == "ABSENT":
...
def hr_employee_delete(emp_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    if not emp:
        raise HTTPException(status_code=404, detail="Not found")
    # delete logs first to avoid FK issues
